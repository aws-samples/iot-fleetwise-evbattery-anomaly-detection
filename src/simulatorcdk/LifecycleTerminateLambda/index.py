...
def find_cluster_name(ec2_c, instance_id):
    ...
    files.
    On failure we raise an exception which means this instance isn't an ECS
    ...
    clustername = re.search("ECS_CLUSTER\s?=\s?(.*?)\s", userdata.decode('utf-8'))
    if clustername:
        ...
        for service in services:
            ...
            for service_status in response["services"]:
                ...
                if re.search(
                        ...
                        service_status["events"][0]["message"].decode('utf-8')
                ):
                    service_ready = True
                    continue
...
def lambda_handler(event, context):
    ...
    # Otherwise this is a HeartBeat call
        ...
        hook_message["AutoScalingGroupName"] = hook_message["autoScalingGroupName"]
        hook_message["LifecycleActionToken"] = hook_message["lifecycleActionToken"]
